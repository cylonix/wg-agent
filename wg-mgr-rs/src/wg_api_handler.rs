...
fn extract_table_id_from_namespace(namespace: &str) -> Result<(&str, u32)> {
    ...
    if let Some(table_id) = table_id {
        let table_id = table_id.parse::<u32>().map_err(|e| {
            Error::new(
...
#[async_trait]
impl<T> ApiHandler for T
where
    T: AsRef<Client> + AsRef<Arc<Mutex<NetworkConfClient>>> + Send + Sync,
{
    async fn delete_namespace_handler(&self, namespace: &WgNamespace) -> Result<()> {
        // first we should search the namespace in db
        let namespace_name = namespace.name.to_owned();

        let etcd_handler: &Client = self.as_ref();
        let ret = etcd_handler.get_namespace(namespace_name.as_str()).await;

        let ns_detail;
        if let Err(e) = ret {
            warn!("Cannot find namespace {} in local store", namespace_name);
            return Err(e);
        } else {
            ns_detail = ret.as_ref().unwrap();
        }

        // delete the wg interfce
        let _delete_wg_ret = match self.delete_wg_interface(ns_detail.name.as_str()) {
        ...
        // delete the vxlan interfce
        let _delete_vxlan_ret = {
            let vxlan = ns_detail.vxlan.as_ref().unwrap();
            self.delete_vxlan_interface(std::format!("{}_{}", "vxlan", vxlan.vid).as_str())
                .map_or_else(|e| Err(e), |_| Ok(()))
        };

        // delete the vrf interface
        let _delete_vrf_ret = {
            let vrf_name = format!("vrf_{}", ns_detail.name);
            self.delete_vrf_interface(vrf_name.as_str())
        };

        let _delete_namespace_store_ret = {
            let etcd_handler: &Client = self.as_ref();
            let delete_namespace_ret = etcd_handler.delete_namespace(namespace_name.as_str()).await;
            // Try to delete all the user configurations with the namespace
            let delete_all_users = etcd_handler
                .delete_all_users_with_namespace(namespace_name.as_str())
                .await;

            let delete_all_users = delete_all_users.map_err(|e| {
                warn!(
                    "Cannot delete all users with the namespace{}, reason:{}",
                    namespace_name,
                    e.to_string()
                );
                e
            ...
            let delete_namespace_ret = delete_namespace_ret
                .map_err(|e| {
                    warn!(
                        "Cannot delete the namespace{}, reason:{}",
                        namespace_name,
                        e.to_string()
                    );
                    e
            ...
            delete_all_users.and(delete_namespace_ret)
        };

        Ok(())
    }

    /// create_namespace_handler will create the namespace in etcd and create the wg interface
    /// and the vxlan interface
    async fn create_namespace_handler(
        &self,
        namespace: &WgNamespace,   // the namespace to be created
        skip_store_into_db: bool,  // if true, we will not store the namespace into etcd
        skip_if_exist_in_db: bool, // if true, we will skip to creat it if the namespace already exists in etcd
        reuse_wg: bool,            // not create the wg interface if it is true
    ) -> Result<()> {
        let wg_interface = namespace.name.to_owned();

        // If we are restoring from DB or repeated creation,
        // we need to check if wg interface has existed so that
        ...
        if reuse_wg {
            match collect_wireguard_info(namespace.name.as_str()) {
                Ok(wd) => {
                    private_key = wd
                        .get_base64_from_sk()
                        .map_or_else(|_e| "None".to_string(), |k| k);
                    debug!(
                        "{} existing wg interface sk: {}",
                        namespace.name, private_key
                    );
        ...
        if create_wg {
            ...
            match self.create_wg_interface(
                ...
                Err(e) => {
                    if e.kind() != std::io::ErrorKind::AlreadyExists {
                        ...
                        debug!("{} new wg interface exists", namespace.name);
                    }
                }
                _ => (),
            }
        ...
        // FIXME: the idea of storing private key for wg users/namespace in etcd
        // does not seem OK. Need a better way to store sensitive information
        // or follow the model of having the private key always locally stored
        ...
        if !skip_store_into_db || force_store_in_db {
            ...
            let etcd_handler: &Client = self.as_ref();
            ret = etcd_handler
                .save_into_namespace(namespace.name.as_str(), namespace_json.as_str())
                .await;
        }

        if let Err(e) = ret {
            warn!(
                "Cannot save namespace{} into store, reason: {}",
                namespace.name,
                e.to_string()
            );
            return Err(e);
        ...
        let ret = if let Err(e) = ret {
            ...
            if let Err(e) = ret {
                if e.kind() == std::io::ErrorKind::AlreadyExists {
                    ...
                    error!(
                        "Cannot create vxlan interface for namespace {}, reason: {}",
                        namespace.name,
                        e.to_string()
                    );
                    Err(e)
        ...
        let ret = if let Err(e) = ret {
            error!(
                "Cannot get table id from namespace {}, reason: {}",
                namespace.name,
                e.to_string()
            );
            Err(e)
        } else {
            table_id = ret.as_ref().unwrap().1;
            let vrf_name = ret.as_ref().unwrap().0;
            debug!("{} table id: {}", namespace.name, table_id);
            let ret = self.create_vrf_interface(format!("vrf_{}", vrf_name), table_id);
            ret
        };
        ...
        let ret = if let Err(e) = ret {
            ...
            let wg_intf_index_ret = self.get_interface_index(namespace.name.as_str());
            let wg_intf_index;
            if let Err(e) = wg_intf_index_ret {
                error!(
                    "Cannot get wg interface index for namespace {}, reason: {}",
                    namespace.name,
                    e.to_string()
                );
                return Err(e);
            ...
            let ret = self.move_interface_to_vrf(wg_intf_index, vrf_index);
            if let Err(e) = ret {
                error!(
                    "Cannot put wg interface into vrf {}, reason: {}",
                    namespace.name,
                    e.to_string()
                );
                return Err(e);
            ...
            let vxlan_if_index_ret =
                self.get_interface_index(format!("vxlan_{}", vxlan.vid).as_str());
            let vxlan_if_index;
            if let Err(e) = vxlan_if_index_ret {
                error!(
                    "Cannot get vxlan interface index for namespace {}, reason: {}",
            ...
            let ret = self.move_interface_to_vrf(vxlan_if_index, vrf_index);
            if let Err(e) = ret {
                error!(
                    "Cannot put vxlan interface into vrf {}, reason: {}",
                    format!("vxlan_{}", vxlan.vid),
                    e.to_string()
                );
                return Err(e);
        ...
        let ret = if let Err(e) = ret {
            ...
            // create the ip mangle tables rule to set fwmark
            let ip = std::format!("{}/{}", wg_interface_ip, wg_interface_prefix);
            let ret = self.create_fwmark_entry(wg_interface, ip, vxlan.vid as u32);
        ...
        let ret = if let Err(e) = ret {
            ...
            let ip = std::format!("{}/{}", wg_interface_ip, wg_interface_prefix);
            let ret = self.create_filter_forward_entry(ip);
        ...
        // add ip routes to the vxlan routing table so that all upstream traffic
        // is forwarded to the vxlan tunnel directly.
        let ret = if let Err(e) = ret {
            Err(e)
        } else {
            // create the default route to the vxlan tunnel
            ...
            if let Err(e) = ret {
                if e.kind() != std::io::ErrorKind::AlreadyExists {
                    *(&mut config_result) = Err(e);
                }
            }

            // config the underlay route so that traffic can really go out
            if let Ok(_) = config_result {
                // FIXME, currenly we use the interface as the underlay route,
                // we shoud change it to gateway
                if vxlan.underlay_if == "" {
                    warn!("configuration underlay if is nil");
                    let err_type = std::io::ErrorKind::InvalidInput;
                    let err_msg = "underlay interface is nil";
                    return Err(std::io::Error::new(err_type, err_msg))
                }

                let if_name = vxlan.underlay_if.clone();
                let ip = std::format!("{}/{}", vxlan.remote, 32);
                let ret = self.create_route_entry(
                    ip, None, Some(if_name), Some(vxlan.vid as u32),
                );
                if let Err(e) = ret {
                    if e.kind() != std::io::ErrorKind::AlreadyExists {
                        *(&mut config_result) = Err(e);
    ...
    async fn get_namespace_stats(
        &self,
        namespace: String,
    ) -> Result<models::InterfaceStats> {
        let stats = self.get_interface_stats(namespace.as_str())?;
    ...
    async fn create_wireguard_user(
        ...
        if let Ok(wg_user_in_db) = etcd_handler
            .get_user(wg_user.namespace.as_str(), wg_user.id.as_str())
            .await
        {
            ...
            if pk == wg_user.pubkey {
                ...
                let _ = etcd_handler
                    .delete_pk(namespace.as_str(), pk.as_str())
                    .await;
                let _ = self.delete_wg_user(&namespace, &pk);
            }
        }

        self.create_wireguard_peer(&wg_user.namespace, &wg_user.pubkey, &wg_user.allowed_ips)?;
        // save it in backend database
        debug!(
            "User [{}/{}/{}] has been created.",
            wg_user.name, wg_user.id, wg_user.pubkey
        );
    ...
    async fn delete_wireguard_user(&self, wg_user: &WgUser) -> Result<()> {
        ...
        let user = etcd_handler
            .get_user(wg_user.namespace.as_str(), wg_user.id.as_str())
            .await?;

        self.delete_wg_user(&wg_user.namespace, &user.pubkey)?;
        debug!(
            "User [{}/{}/{}] has been deleted.",
            wg_user.namespace, wg_user.id, user.pubkey,
        );
        // delete it from backend database
        let _ = etcd_handler
            .delete_user(wg_user.namespace.as_str(), user.id.as_str())
            .await?;
        etcd_handler
            .delete_pk(wg_user.namespace.as_str(), user.pubkey.as_str())
            .await
    }
    ...
    async fn get_user_statistics(&self, wg_user: &WgUser) -> Result<models::WgUserStats> {
        ...
        let ret = self.get_all_users(namespace).and_then(|peers| {
            ...
            match a {
                None => {
                    error!(
                        "get user stats: user {}/{} not found",
                        wg_user.name, wg_user.pubkey
                    );
                    Err(Error::new(ErrorKind::Other, "User not found"))
                }
                Some(a) => {
                    info!(
                        "get user stats: user {}/{} found",
                        wg_user.name, wg_user.pubkey
                    );
                    Ok(a)
                }
    ...
    async fn get_all_user_stats(&self, namespace: &str) -> Result<Vec<models::WgUserStats>> {
        ...
        let peers = self.get_all_users(namespace)?;
                ...
                for peer in peers.into_iter() {
                    ...
                    match etcd_handler.get_pk(namespace, pk.as_str()).await {
                        ...
                        Ok(user_id) => {
                            ...
                            let user = etcd_handler.get_user(namespace, user_id.as_str()).await;
                            match user {
                                Err(_e) => to_delete = true,
                                Ok(u) => to_delete = u.pubkey != pk,
                            }
                            if to_delete {
